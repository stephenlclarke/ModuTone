...
async fn download_model_files(
    ...
    let mut downloaded = existing_complete_bytes(&user_models_dir, spec);
    ...
    for file in spec.files {
        ...
        if is_complete_file(&destination, file.size_bytes) {
            emit_progress(
                app,
                model_id,
...
async fn download_file(
    ...
    let partial_existing_size = file_size(&partial).unwrap_or(0);
    if partial_existing_size == file.size_bytes {
        tokio::fs::rename(&partial, destination)
            ...
            .map_err(|e| DownloadFailure::Failed {
                ...
                error: format!(
                    "Failed to install complete partial {} to {}: {}",
                    partial.display(),
                    destination.display(),
                    e
                ),
...
fn existing_complete_bytes(root: &Path, spec: &DownloadSpec) -> u64 {
    spec.files
        .iter()
        .filter_map(|file| {
            safe_join(root, file.relative_path)
                .ok()
                .filter(|path| is_complete_file(path, file.size_bytes))
                .map(|_| file.size_bytes)
        })
        .sum()
}

fn is_complete_file(path: &Path, expected_size: u64) -> bool {
    file_size(path).is_some_and(|size| size >= expected_size)
}
